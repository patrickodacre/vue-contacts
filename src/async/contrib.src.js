...
export default ((ASQ) => {
  ...
  (function IIFE() {
    ...
    ASQ.iterable = function $$iterable() {
      ...
      function next() {
        ...
        try {
          return {
            value: val_queue.shift().apply(ø, arguments)
          };
        } catch (err) {
  ...
  ASQ.extend("pThen", function $$extend(api, internals) {
    return function $$pthen(success, failure) {
      ...
      var ignore_success_handler = false,
        ignore_failure_handler = false;
